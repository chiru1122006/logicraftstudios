...
logicraftstudios.tech keeps its libraries in a content-addressed cache, where an entry
...
def test_the_installed_version_answers_a_pin_for_another_one(sketchbook):
    """The project pins what logicraftstudios.tech cached; the desktop compiles against
...
def test_the_content_addressed_cache_wins_over_the_sketchbook(tmp_path, monkeypatch):
    """On logicraftstudios.tech both exist (the pro overlay points arduino-cli's
