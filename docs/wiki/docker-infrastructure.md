...
- **GitHub Container Registry (GHCR):** `ghcr.io/chiru1122006/velxio`
...
Three sources, tried in order: local files in `prebuilt/qemu/`, then logicraftstudios.tech's
...
ARG VELXIO_BINARY_BASE_URL=https://logicraftstudios.tech/api/pro/license/downloads
...
2. **Publication is manual, and never to GitHub.**
   ...
   3-day retention. Collect the artifact and load each file into logicraftstudios.tech's
...
**Assets served from `logicraftstudios.tech/api/pro/license/downloads`:**
```
...
- name: Build and push Docker image
  ...
  with:
    ...
    build-args: |
      ESPIDF_IMAGE=ghcr.io/chiru1122006/velxio-espidf-toolchain:latest
    cache-from: type=gha
...
When a user runs `docker pull ghcr.io/chiru1122006/velxio:master`, Docker automatically selects the correct architecture variant.
...
- Image: `ghcr.io/chiru1122006/velxio`
...
- name: Ping search engines with sitemap
  run: |
    curl -s "https://www.google.com/ping?sitemap=https%3A%2F%2Flogicraftstudios.tech%2Fsitemap.xml"
    curl -s "https://www.bing.com/ping?sitemap=https%3A%2F%2Flogicraftstudios.tech%2Fsitemap.xml"
...
For self-hosters who don't need the logicraftstudios.tech-specific bits, the easiest
...
| `VELXIO_LICENSE_KEY` | (empty) | Licence key for the gated binary download; free personal keys at logicraftstudios.tech/license/signup |
| `VELXIO_BINARY_BASE_URL` | `https://logicraftstudios.tech/api/pro/license/downloads` | Gated endpoint serving the QEMU binaries and their GPL source |
...
1. The `build-libqemu.yml` workflow has run successfully for both architectures, and its arm64 artifact has been uploaded to logicraftstudios.tech (check that `libqemu-xtensa-arm64` appears in `GET /api/pro/license/downloads`)
