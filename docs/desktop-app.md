...
> The desktop app is a **Pro** feature (30-day free trial). The OSS web build remains free at logicraftstudios.tech and via Docker self-host.
...
Download the installer for your platform from [logicraftstudios.tech/download](https://logicraftstudios.tech/download):
...
The desktop app uses `vlx_pro_*` / `vlx_trial_*` keys issued by logicraftstudios.tech.
...
4. Your default browser opens `https://logicraftstudios.tech/auth/desktop`.
...
If your browser blocks the custom-URL scheme (some Linux distros, corporate proxies), click **Paste key** on the welcome screen and enter the `vlx_pro_*` or `vlx_trial_*` string from [Account → My licenses](https://logicraftstudios.tech/account/licenses).
...
| `license_validate` | POST it to `https://logicraftstudios.tech/api/pro/license/validate` |
...
- **Pro $15/mo** — desktop app + premium components on logicraftstudios.tech.
- **Pro Max $35/mo** — everything in Pro plus the in-app AI assistant and priority simulation queue.

Manage your subscription at [logicraftstudios.tech/billing](https://logicraftstudios.tech/billing).

---

## Troubleshooting

- **Welcome screen won't close after sign-in** — the deep-link handler didn't fire. Click **Paste key** instead and use the key from [Account → My licenses](https://logicraftstudios.tech/account/licenses).
