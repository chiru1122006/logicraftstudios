...
core as a component. This is what the Docker image and logicraftstudios.tech use, so a
sketch that builds on logicraftstudios.tech builds identically here.
...
there too, or logicraftstudios.tech sketches will not compile:
...
> current libqemu builds run IDF 5.x firmware, and logicraftstudios.tech moved to the
...
**Use: arduino-esp32 3.3.10 on ESP-IDF 5.5.4** (the pair logicraftstudios.tech builds
...
boots on libqemu, but sketches written on logicraftstudios.tech use 3.x APIs and do not
compile against it.
...
docker run -d -p 3080:80 ghcr.io/chiru1122006/velxio:master
