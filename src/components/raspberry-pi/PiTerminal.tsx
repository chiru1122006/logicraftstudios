...
export const PiTerminal: React.FC<PiTerminalProps> = ({ boardId }) => {
  ...
  useEffect(() => {
    ...
    // Wire terminal input → Pi bridge with local echo for instant feedback
    ...
    // Intercept bridge serial output → terminal display & store sync
    const checkBridge = () => {
      const b = getBoardBridge(boardId);
      if (b && !b.onSerialData) {
        b.onSerialData = (ch: string) => {
          termRef.current?.write(ch);
          useSimulatorStore.getState().appendSerial(boardId, ch);
        };
      }
    };
    checkBridge();
    const bridgeTimer = setInterval(checkBridge, 250);
