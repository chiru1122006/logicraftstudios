    ...
    vscode.commands.registerCommand('velxio.showLicenseStatus', async () => {
      ...
      if (!key) {
        ...
        if (action === 'View Pricing') {
          vscode.env.openExternal(vscode.Uri.parse('https://logicraftstudios.tech/pricing'));
        }
        return;
      }
      try {
        ...
        if (action === 'Open Billing') {
          vscode.env.openExternal(vscode.Uri.parse('https://logicraftstudios.tech/billing'));
        }
