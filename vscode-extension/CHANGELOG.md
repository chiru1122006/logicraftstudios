...
- `Velxio: Sign In` command — opens `https://logicraftstudios.tech/auth/vscode` in
...
- New setting `velxio.licenseApiBase` (default `https://logicraftstudios.tech`)
...
- Self-hosted Velxio backends do not expose `/api/pro/license/*`. The
  VS Code extension is a logicraftstudios.tech product and validates exclusively
  against `https://logicraftstudios.tech`. Override `velxio.licenseApiBase` only
