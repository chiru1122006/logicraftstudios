...
export const ComponentInfoPanel: React.FC<ComponentInfoPanelProps> = ({
  ...
  React.useLayoutEffect(() => {
    ...
    // When the component picker is open as a right sidebar, float the datasheet
    // panel cleanly to the left of the sidebar so it never overlaps any of the cards.
    const sidebarEl = document.querySelector('.component-picker-modal');
    if (sidebarEl) {
      const sRect = sidebarEl.getBoundingClientRect();
      if (sRect.right >= vw - 25 && sRect.left < vw) {
        const left = Math.max(margin, sRect.left - w - gap);
