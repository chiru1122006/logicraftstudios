...
export const SimulatorCanvas = ({ headerSlot }: SimulatorCanvasProps = {}) => {
  ...
  useEffect(() => {
    ...
    boards.forEach((board) => {
      if (board.boardKind === 'aries-v3') {
        const checkAriesLed = () => {
          // ARIES v3 onboard RGB LED: Pin 22 (Green), 23 (Blue), 24 (Red) active-low
          // Also supports Pin 10 / LED_BUILTIN active-high
          const p22 = pinManager.getPinState(22);
          const p23 = pinManager.getPinState(23);
          const p24 = pinManager.getPinState(24);
          const p10 = pinManager.getPinState(10);
          const isLedOn = (!p22 && p22 !== undefined) || (!p23 && p23 !== undefined) || (!p24 && p24 !== undefined) || Boolean(p10);
          setBoardLedStates((prev) => {
            if (prev[board.id] === isLedOn) return prev;
            return { ...prev, [board.id]: isLedOn };
          });
        };
        unsubs.push(pinManager.onPinChange(22, checkAriesLed));
        unsubs.push(pinManager.onPinChange(23, checkAriesLed));
        unsubs.push(pinManager.onPinChange(24, checkAriesLed));
        unsubs.push(pinManager.onPinChange(10, checkAriesLed));
        return;
      }
            ...
            <div className={`canvas-header${headerSlot ? ' canvas-header--portaled' : ''}`}>
              <div className="canvas-header-left">
                ...
                {/* Overlay slot for the WiFi/network panel (local gateway
                    pairing on logicraftstudios.tech). Empty in the OSS build. */}
                ...
                {activeBoard &&
                  ...
                  (() => {
                    ...
                    const openGateway = () => {
                      ...
                      // A private overlay (logicraftstudios.tech) can install a synchronous
