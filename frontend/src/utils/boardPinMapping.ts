...
export function boardPinToNumber(boardId: string, pinName: string): number | null {
  ...
  // C-DAC ARIES v3.0 (THEJAS32 RISC-V SoC)
  if (boardId === 'aries-v3' || boardId.startsWith('aries-v3')) {
    if (
      pinName.startsWith('GND') ||
      pinName.startsWith('3V3') ||
      pinName.startsWith('3.3V') ||
      pinName.startsWith('5V') ||
      pinName.startsWith('VCC') ||
      pinName === 'VIN' ||
      pinName === 'IOREF' ||
      pinName === 'RST' ||
      pinName === 'RESET' ||
    ...
    // Direct GPIO names: GPIO0..GPIO31
    const mGpio = /^GPIO(\d+)$/.exec(pinName);
    if (mGpio) {
      const g = parseInt(mGpio[1], 10);
      if (g >= 0 && g <= 31) return g;
    }
    // Analog input pins: A0..A3 or ADC_CH0..ADC_CH3
    if (/^A(\d+)$/.test(pinName)) {
      return 32 + parseInt(pinName.substring(1), 10);
    }
    const mAdc = /^ADC_CH(\d+)$/.exec(pinName);
    if (mAdc) {
      return 32 + parseInt(mAdc[1], 10);
    }
    // PWM aliases: PWM0..PWM7
    const mPwm = /^PWM(\d+)$/.exec(pinName);
    if (mPwm) {
      const p = parseInt(mPwm[1], 10);
      const pwmToGpio = [0, 1, 2, 13, 14, 15, 16, 17];
      return pwmToGpio[p] ?? p;
    }
    // Numeric fallback (0..31)
    const num = parseInt(pinName, 10);
