...
function validatePayload(data: unknown): VlxPayload {
  ...
  if (data.format !== VLX_FORMAT) {
    throw new VlxParseError(
      `Not a Logicraft Studios project file (expected format="${VLX_FORMAT}", got ${JSON.stringify(
        data.format,
      )}).`,
    );
  }
  ...
  if (data.version > VLX_VERSION) {
    throw new VlxParseError(
      `This file uses .vlx format version ${data.version}, but this Logicraft Studios supports up to v${VLX_VERSION}. Update Logicraft Studios to open it.`,
    );
  }
