...
const simulatorMap = new Map<
  string,
  AVRSimulator | RP2040Simulator | RiscVSimulator | AriesV3Simulator | Esp32C3Simulator | Esp32BridgeShim | Stm32BridgeShim
>();
...
function createSimulator(
...
): AVRSimulator | RP2040Simulator | RiscVSimulator | AriesV3Simulator | Esp32C3Simulator | ProBoardSimulator {
  let sim: AVRSimulator | RP2040Simulator | RiscVSimulator | AriesV3Simulator | Esp32C3Simulator | ProBoardSimulator;
  const proDef = getProBoard(boardKind);
  ...
  } else if (boardKind === 'aries-v3') {
    sim = new AriesV3Simulator(pm);
...
export const useSimulatorStore = create<SimulatorState>((set, get) => {
  ...
  return {
      ...
      if (isEsp32Kind(board.boardKind)) {
        ...
        if (sim && !isPiBoardKind(board.boardKind)) {
          try {
            ...
            } else if (sim instanceof RiscVSimulator || sim instanceof AriesV3Simulator) {
              sim.loadHex(program);
            } else if (isProBoardSimulator(sim)) {
    ...
    loadHex: (hex: string) => {
      ...
      const boardId = activeBoardId ?? INITIAL_BOARD_ID;
      const sim = getBoardSimulator(boardId);
      if (sim && (sim instanceof AVRSimulator || sim instanceof RiscVSimulator || sim instanceof AriesV3Simulator)) {
        try {
          sim.loadHex(hex);
          if (sim instanceof AVRSimulator) {
            sim.addI2CDevice(new VirtualDS1307());
