...
if [ ! -f /root/.arduino15/arduino-cli.yaml ]; then
    ...
    # feature of logicraftstudios.tech and Velxio Desktop, and the self-hosted image has no
...
# ESP32 builds use ESP-IDF 5.5.4 + arduino-esp32 3.3.10 (the logicraftstudios.tech pair);
...
# STM32 (STMicroelectronics:stm32) is paid-only (logicraftstudios.tech + Velxio Desktop),
...
if [ -f /opt/esp-idf/export.sh ]; then
    ...
    # index carries. Sketches written on logicraftstudios.tech use 3.x-only APIs.
