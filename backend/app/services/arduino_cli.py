...
def _discard_sketch_build_cache(sketch_dir: Path) -> None:
    ...
    on logicraftstudios.tech: 17,000 directories and 15 GB in 15 days, growing 1 GB a
...
class ArduinoCLIService:
    ...
    CORE_URLS: dict[str, str] = {
        ...
        "vega:riscv": "https://gitlab.com/riscv-vega/vega-arduino/-/raw/main/package_vega_index.json",
    }
    ...
    def _is_vega_board(self, fqbn: str) -> bool:
        """Return True if the FQBN targets a C-DAC VEGA RISC-V board (e.g. ARIES v3)."""
        return "vega:riscv" in fqbn or "cdac:riscv" in fqbn
    ...
    async def compile(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for file_entry in files:
                ...
                # VEGA RISC-V (ARIES v3.0, etc.): The VEGA Arduino core leaves `TwoWire Wire` uninstantiated
                # in Wire.cpp because it supports multiple I2C controllers (Wire(0) on I2C-0, Wire(1) on I2C-1).
                # Standard Arduino libraries (Adafruit SSD1306, sensors, LCDs) expect
                # `Wire` to exist. If the sketch references Wire or I2C but hasn't instantiated
                # TwoWire Wire, auto-instantiate TwoWire Wire(1) (default I2C-1 on SDA1/SCL1)
                # so standard Arduino sketches compile seamlessly without link errors.
                if self._is_vega_board(board_fqbn) and write_name == "sketch.ino":
                    all_sketch_text = "\n".join(f.get("content", "") for f in files)
                    has_twowire_inst = "TwoWire Wire" in all_sketch_text
                    uses_wire = "Wire" in all_sketch_text
                    if uses_wire and not has_twowire_inst:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    elif self._is_vega_board(board_fqbn):
                        hex_file = build_dir / "sketch.ino.hex"
                        bin_file = build_dir / "sketch.ino.bin"
                        elf_file = build_dir / "sketch.ino.elf"

                        hex_content = hex_file.read_text(encoding="utf-8") if hex_file.exists() else None
                        target_file = bin_file if bin_file.exists() else (elf_file if elf_file.exists() else None)
                        binary_b64 = base64.b64encode(target_file.read_bytes()).decode('ascii') if target_file else None

                        if hex_file.exists() or target_file:
                            print(f"[VEGA] Build artifacts: hex={hex_file.exists()}, bin={bin_file.exists()}, elf={elf_file.exists()}")
                            print("=== VEGA RISC-V Compilation successful ===\n")
                            return {
                                "success": True,
                                "hex_content": hex_content,
                                "binary_content": binary_b64,
                                "binary_type": "bin" if bin_file.exists() else ("elf" if elf_file.exists() else "hex"),
                                "stdout": result.stdout,
                                "stderr": result.stderr,
                            }
                        else:
                            return {
                                "success": False,
                                "error": "VEGA firmware (.hex/.bin) not found after compilation",
                                "stdout": result.stdout,
                                "stderr": result.stderr,
                            }
                    else:
    ...
    async def uninstall_library(self, library_name: str) -> dict:
        ...
        `list_installed_libraries` does. On logicraftstudios.tech that sketchbook's
